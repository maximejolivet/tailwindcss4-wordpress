...
3. **`.env` de production** : n'existe que sur le serveur, jamais versionné,
   ...
   (utilise les valeurs de `.env.deploy`, dont `SENTRY_DSN` — cf.
   `web/app/mu-plugins/sentry.php` — génère des salts WordPress fraîches,
   affiche un aperçu avant de le copier via `scp`).
...
3. **Production `.env`**: only exists on the server, never versioned, never
   ...
   values from `.env.deploy`, including `SENTRY_DSN` — see
   `web/app/mu-plugins/sentry.php` — generates fresh WordPress salts, shows
   a preview before copying it via `scp`).
