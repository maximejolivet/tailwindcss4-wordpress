...
3. Create `<name>.twig`:
   ...
   - Slots needed → use `{% block name %}{% endblock %}` (never `{{ name }}` interpolation, which can't carry markup or nested components), meant to be consumed via `{% embed %}`. Document in the README that callers must **never** add `only` to the `{% embed %}` tag itself (it silently cuts off `post`/`site` access inside the slot blocks — see @.claude/WORDPRESS.md) — leaf `{% include %}` calls nested inside a slot keep their own `only`.
